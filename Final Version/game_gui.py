...
class GUI():    
    def __init__(self):
        self.github_path = "https://raw.githubusercontent.com/st21425/AS91907/main/RogueRollerv2/"
    ...
    def menu(self):
        ...
        self.game_button = Button(frame, text="New Game", bg=self.button_colour, font=self.font, command=lambda: [self.gui_reset(), self.update_sidebar(), self.show_frames("GameMenu"), self.play_turn()])
    ...
    def sidebar(self, master):
        ...
        self.round_label = Label(frame, font = self.font, text = f"Round: {self.round}", bg=self.sidebar_colour, fg=self.sidebar_text)
        ...
        help_button = Button(frame, text="Help", bg=self.sidebar_box_colour, fg=self.sidebar_text, font=self.font, command=lambda: self.show_help("help_image.png"))
        help_button.grid(padx=75, pady=10, sticky=EW)
    ...
    def shop_content(self, master):
        ...
        for item in self.items_in_shop:
            ...
            self.next_round_button = Button(frame, text="Next Round", bg=self.button_colour, font=self.large_font, command=lambda: [self.update_sidebar(),self.gui_next_round(), self.update_sidebar(), self.play_turn()])
    ...
    def tooltip(self, location):
        for i in range(len(location)):
            ...
            tooltip_text = tooltip_name + tooltip_type + tooltip_sides
            Hovertip(anchor_widget=location[i],text=tooltip_text,hover_delay=None)
    ...
    def show_help(self, help_image_path):
        ...
        # Load the help image
        self.help_image = PhotoImage(file=help_image_path)
        help_label = Label(self.help_overlay, image=self.help_image)
        help_label.pack(expand=True)

        # Add close button
        close_button = Button(self.help_overlay, text="Back", font=self.font, bg=self.button_colour, command=self.close_help)
