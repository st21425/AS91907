...
class GUI():
    def __init__(self):
        self.github_path = (
            "https://raw.githubusercontent.com/st21425/AS91907/main/Final%20Version/")
        ...
        self.dice_type_info = self.logic.load_json("dice_types.json")

        # load images
        self.dice_image = self.image_loader("clear.png")
        self.locked_dice_image = self.image_loader("lock_dice.png")
        self.help_menu_image = self.image_loader("help_menu.png")
        self.hands_menu_image = self.image_loader("hand_menu.png")

        self.hands_menu_image = self.hands_menu_image.subsample(2)
        self.help_menu_image = self.help_menu_image.subsample(2)
        ...
        self.button_colour = "#62A0A7"
        self.bg_colour = "#97C0C4"
    ...
    def menu(self):
        ...
        self.title_label = Label(
            frame, font=self.large_font, text="Rogue Roller", bg=self.bg_colour)
        self.title_label.grid(padx=1, pady=150, sticky=NSEW)

        self.game_button = Button(frame, text="New Game", bg=self.button_colour,
                                  font=self.font, command=lambda: [
                                      self.gui_reset(), self.update_sidebar(),
                                      self.show_frames("GameMenu"), self.play_turn()])
        self.game_button.grid(padx=350, pady=10, sticky=NSEW)

        self.quit_button = Button(
            frame, text="Quit", bg=self.button_colour, font=self.font, command=lambda: self.quit())
    ...
    def game(self):
        ...
        self.dice = self.logic.load_json("dice.json")
    ...
    def game_lose(self):
        ...
        self.title_label = Label(
            frame, font=self.large_font, text="Game Over", bg=self.bg_colour)
        self.title_label.grid(padx=1, pady=150, sticky=NSEW)

        self.game_button = Button(frame, text="New Game",
                                  bg=self.button_colour, font=self.font,
                                  command=lambda: [
                                      self.gui_reset(), self.update_sidebar(),
    ...
    def sidebar(self, master):
        ...
        self.round_label = Label(
            frame, font=self.font, text=f"Round: {self.round}", bg=self.sidebar_colour, fg=self.sidebar_text)
        self.round_label.grid(padx=100, pady=10, sticky=EW)

        self.requirement_label = Label(
            frame, font=self.font, text=f"Requirement: {self.round_requirement}", bg=self.sidebar_colour, fg=self.sidebar_text)
        self.requirement_label.grid(padx=100, pady=10, sticky=EW)

        self.total_label = Label(
            frame, font=self.font, text=f"Total: {self.total}", bg=self.sidebar_box_colour, fg=self.sidebar_text)
        self.total_label.grid(padx=75, pady=5, sticky=EW)

        self.scoring_label = Label(
            frame, font=self.font, text=f"Remaining Hands: {self.max_hands}", bg=self.sidebar_box_colour, fg=self.sidebar_text)
        self.scoring_label.grid(padx=75, pady=5, sticky=EW)

        self.money_label = Label(
            frame, font=self.font, text=f"Money: {self.money}", bg=self.sidebar_box_colour, fg=self.sidebar_text)
        self.money_label.grid(padx=75, pady=5, sticky=EW)

        self.menu_button = Button(frame, text="Menu", bg=self.sidebar_box_colour,
                                  fg=self.sidebar_text, font=self.font, command=lambda: self.show_frames("MainMenu"))
        self.menu_button.grid(padx=75, pady=15, sticky=EW)

        self.hands_button = Button(frame, text="Hands", bg=self.sidebar_box_colour, fg=self.sidebar_text,
                                   font=self.font, command=lambda: self.show_help(self.hands_menu_image))
        self.hands_button.grid(padx=75, pady=10, sticky=EW)

        self.help_button = Button(frame, text="Help", bg=self.sidebar_box_colour, fg=self.sidebar_text,
                                  font=self.font, command=lambda: self.show_help(self.help_menu_image))
        self.help_button.grid(padx=75, pady=10, sticky=EW)
    ...
    def game_content(self, master):
        ...
        for y in range(2):
            for x in range(3):
                ...
                self.dice_button = Button(frame, image=self.dice_image, text=self.played[x + y*3], bg=bg_colour, font=self.large_font,
                                          compound=CENTER, borderwidth=0, command=lambda value=(x + 1 + y*3): self.toggle_dice(value))
                self.dice_button.grid(row=y, column=x, padx=100)
                self.dice_buttons.append(self.dice_button)

        self.tooltip(self.dice_buttons)

        play_button = Button(frame, text="Play Hand", bg=self.button_colour, font=self.large_font,
                             compound=CENTER, borderwidth=0, command=lambda: self.play_turn())
    ...
    def shop_content(self, master):
        ...
        self.shop_label = Label(
            frame, font=self.large_font, text="Shop", bg=self.bg_colour)
        ...
        for item in self.items_in_shop:
            ...
            self.next_round_button = Button(frame, text="Next Round", bg=self.button_colour, font=self.large_font, command=lambda: [
                                            self.update_sidebar(), self.gui_next_round(), self.update_sidebar(), self.play_turn()])
    ...
    def current_dice(self, master):
        ...
        self.current_dice_label = Label(
            frame, font=self.large_font, text="Current Dice", bg=self.bg_colour)
        self.current_dice_label.grid(
            padx=1, pady=15, sticky=NSEW, row=0, columnspan=7)

        self.current_dice_labels = []

        # make the current dice in a line
        for x in range(6):
            dice_type = self.dice["die" + str(x + 1)]["type"]
            bg_colour = self.dice_type_info[dice_type]["colour"]
            self.dice_button = Button(frame, image=self.shop_dice_image,
                                      text=f"Dice {x + 1}", bg=bg_colour, font=self.font, compound=CENTER, borderwidth=0)
            self.dice_button.grid(column=x, row=1, padx=10)
            self.current_dice_labels.append(self.dice_button)

        self.tooltip(self.current_dice_labels)
    ...
    def replace_dice(self):
        ...
        self.current_dice_label = Label(
            frame, font=self.large_font, text="Select a dice to replace", bg=self.bg_colour)
        self.current_dice_label.grid(
            padx=1, pady=15, sticky=NSEW, row=0, columnspan=7)
        ...
        for y in range(2):
            for x in range(3):
                ...
                dice_type = self.dice["die" + str(x + 1 + y*3)]["type"]
                bg_colour = self.dice_type_info[dice_type]["colour"]
                self.dice_button = Button(frame, image=self.dice_image, text=f"Dice {x +1 + y*3} \nFaces: {faces}", bg=bg_colour, font=self.font,
                                          compound=CENTER, borderwidth=0, wraplength=150, justify=CENTER, command=lambda value=(x + 1 + y*3): self.dice_change(value))
    ...
    def update_sidebar(self):
        ...
        self.round_label.config(text=f"Round: {data['round']}")
        self.requirement_label.config(
            text=f"Requirement: {data['requirement']}")
    ...
    def gui_reset(self):
        ...
        for i in range(6):
            ...
            self.button_update.config(image=self.dice_image)
        for i in range(6):
            dice_type = self.dice["die" + str(i + 1)]["type"]
            bg_colour = self.dice_type_info[dice_type]["colour"]
            update_game_button = self.dice_buttons[(i)]
            update_game_button.config(bg=bg_colour)
            update_shop_button = self.current_dice_labels[(i)]
            update_shop_button.config(bg=bg_colour)
    #prepare buy dice
    def buy_dice(self, value):
        self.can_buy = self.logic.check_price(value)
        if self.can_buy == True:
            self.show_frames("ReplaceDice")
    #refresh dice shown in shop
    def refresh_shop_dice(self):
        item = self.logic.get_gui_data()["shop_dice"]
        for i in range(len(self.shop_dice_buttons)):
            dice_type = item[i]['type']
            bg_colour = self.dice_type_info[dice_type]["colour"]
            button_to_update = self.shop_dice_buttons[i]
            button_to_update.config(
                text=f"{item[i]['name']} \nCost: {item[i]['cost']}", bg=bg_colour)
        return item
    #change dice visually
    def dice_change(self, value):
        self.logic.dice_change(value)
        self.update_sidebar()
        self.tooltip(self.dice_buttons)
        self.tooltip(self.current_dice_labels)
        dice_type = self.dice["die" + str(value)]["type"]
        bg_colour = self.dice_type_info[dice_type]["colour"]
        update_game_button = self.dice_buttons[(value - 1)]
        update_game_button.config(bg=bg_colour)
        update_shop_button = self.current_dice_labels[(value - 1)]
        update_shop_button.config(bg=bg_colour)
        self.show_frames("ShopMenu")
    #show the game win choose new dice
    def add_dice(self):
        frame = Frame(self.container)
        frame.configure(bg=self.bg_colour)
        ...
        self.add_dice_label = Label(
            frame, text="You can now add a dice", font=self.large_font, bg=self.bg_colour)
        self.add_dice_label.grid(padx=10, pady=15, sticky=NSEW, row=0)

        self.name_label = Label(frame, text="Dice name",
                                font=self.font, bg=self.bg_colour)
        self.name_label.grid(padx=10, pady=15, sticky=NSEW, row=1)

        self.name_entry = Entry(frame, font=self.font, bg=self.button_colour)
        self.name_entry.grid(padx=400, pady=15, sticky=NSEW, row=2)

        self.type_label = Label(frame, text="Dice type",
                                font=self.font, bg=self.bg_colour)
        ...
        self.type_of_dice = ttk.Combobox(
            frame, font=self.font, values=dice_types, state="readonly", textvariable=self.combo_default)
        self.type_of_dice.grid(padx=400, pady=15, sticky=NSEW, row=4)

        self.faces_label = Label(
            frame, text="Number of die faces (2-20)", font=self.font, bg=self.bg_colour)
        self.faces_label.grid(padx=10, pady=15, sticky=NSEW, row=5)

        self.faces_entry = Entry(frame, font=self.font, bg=self.button_colour)
        self.faces_entry.grid(padx=400, pady=15, sticky=NSEW, row=6)

        self.cost_label = Label(
            frame, text="Cost of dice (1-5)", font=self.font, bg=self.bg_colour)
        self.cost_label.grid(padx=10, pady=15, sticky=NSEW, row=7)

        self.cost_entry = Entry(frame, font=self.font, bg=self.button_colour)
        self.cost_entry.grid(padx=400, pady=15, sticky=NSEW, row=8)

        self.continue_dice = Button(frame, text="Choose face numbers", bg=self.button_colour,
                                    font=self.font, command=lambda: self.check_new_dice())
        ...
        try:
            ...
            if self.faces < 2 or self.faces > 20:
                self.faces_label.config(
                    text="Choose a valid number of faces between 2 and 20")
                raise ValueError
            ...
            if self.dice_cost < 1 or self.dice_cost > 5:
                self.cost_label.config(
                    text="Choose a valid cost between 1 and 5")
                raise ValueError
            ...
            self.continue_dice.config(
                text="Ensure all boxes have been filled before you continue")
    # check the users added faces
    def add_faces(self):
        self.faces = 1
        frame = Frame(self.container)
        frame.configure(bg=self.bg_colour)
        ...
        self.add_dice_label = Label(
            frame, text="Choose the faces for the dice", font=self.large_font, bg=self.bg_colour)
        self.add_dice_label.grid(padx=10, pady=15, sticky=NSEW, row=0)

        self.values_label = Label(
            frame, text="Enter the values for each face of the dice (0-100)", font=self.font, bg=self.bg_colour)
        ...
        self.submit_button = Button(frame, text="Submit Dice",
                                    bg=self.button_colour, font=self.font,
                                    command=lambda: self.submit_dice())
        self.submit_button.grid(
            padx=400, pady=10, sticky=NSEW, row=(self.faces + 3))
    ...
    def submit_dice(self):
            ...
            try:
                ...
                if value < 0 or value > 100:
                    self.values_label.config(
                        text="Enter values between 0 and 100")
                    raise ValueError
                values.append(value)
            except ValueError:
                self.values_label.config(
                    text="""Ensure all boxes have been filled with numbers before you continue""")
                return
        self.new_dice = {}
        ...
        self.show_frames("MainMenu")
    #makes a tooltip for the dice
    def tooltip(self, location):
        for i in range(len(location)):
            tooltip_text = self.dice["die" + str(i + 1)]
            tooltip_attributes = ""
            ...
            tooltip_sides = f"Sides: {tooltip_text['sides']}"
            tooltip_text = (tooltip_name + tooltip_type + tooltip_attributes
                            + tooltip_sides)
            Hovertip(anchor_widget=location[i],
                     text=tooltip_text, hover_delay=None)
    #show the help menus
    def show_help(self, help_image_path):
        # Store the current frame name so we can return to it
        ...
        self.help_overlay = Frame(self.container, bg=self.bg_colour)
        self.help_overlay.place(relx=0, rely=0, relwidth=1, relheight=1)

        # Load the help image
        self.help_image = help_image_path
        help_label = Label(self.help_overlay, image=self.help_image)
        help_label.pack(expand=True)

        # Add close button
        close_button = Button(self.help_overlay, text="Back", font=self.font,
                              bg=self.button_colour, command=self.close_help)
    ...
    def close_help(self):
        ...
        if self.previous_frame:
            self.show_frames(self.previous_frame)
    #load images
    def image_loader(self, image):
        try:
            try:
                output = PhotoImage(file=image)
            except Exception as e:
                output = urllib.request.urlretrieve((self.github_path +
                                                     image), image)
                output = PhotoImage(file=image)

        except Exception as error:
            print(f"""An error occured while loading image
                  Image: {image}\nError: {error}""")
        return output
