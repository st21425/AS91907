...
class GUI():    
    ...
    def shop_content(self, master):
        ...
        for item in self.items_in_shop:
            ...
            self.next_round_button = Button(frame, text="Next Round", bg=self.button_colour, font=self.large_font, command=lambda: [self.gui_next_round(), self.update_sidebar(), self.show_frames("GameMenu")])
    ...
    def refresh_shop_dice(self):
        ...
        for i in range(len(self.shop_dice_buttons)):
            ...
            button_to_update.config(text=f"{item['name']} \nCost: {item['cost']}")
